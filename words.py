...
# Remember to make a directory for special_characters so that the bot can refer/generate new words.
...
def word_generator(word):
    ...
    for each_letter in word.lower():
        temp.append(read_json_special_characters()[each_letter])
            ...
            if os.path.isfile(filename):
                with open(filename) as reading_json:
                    ...
                    if word in data.keys():
                        embed = discord.Embed(title="It exists!", description=f"`{word}` is already in the filter list!",
                                              colour=discord.Color.red())
                    else:
                        embed = discord.Embed(title="It does not exist!", description=f"`{word}` is not in the filter list!",
